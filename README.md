# RNA-Seq 후속 분석 파이프라인: GO 및 GSEA# RNA-Seq Post-Analysis Pipeline: GO and GSEA
...
RNA-Seq 차등 발현 분석 후 **Gene Ontology (GO) 농축 분석**과 **Gene Set Enrichment Analysis (GSEA)**를 수행하는 종합적이고 모듈화된 Python 파이프라인입니다. Jupyter 노트북을 통한 인터랙티브 분석과 대규모 배치 처리를 모두 지원합니다.
...
- **🧩 고도로 모듈화됨:** 독립적이고 재사용 가능한 분석 모듈을 자유롭게 조합- **⚙️ Configuration-Driven:** YAML-based configuration for reproducible analyses

- **⚙️ 설정 기반:** YAML 파일로 재현 가능한 분석 수행- **🚀 Multiple Execution Modes:** Interactive notebooks, CLI tools, and Snakemake workflows

- **🚀 다양한 실행 모드:** Jupyter 노트북, CLI 도구, Snakemake 워크플로우 지원- **🐍 Snakemake Integration:** Automated workflow management with parallelization and error recovery

- **🐍 Snakemake 통합:** 병렬 처리 및 에러 복구 기능을 갖춘 자동화된 워크플로우- **📊 Publication-Ready Outputs:** High-quality visualizations and comprehensive HTML reports

- **📊 논문 수준 결과물:** 고품질 시각화 및 종합 HTML 리포트 생성- **🔄 Efficient Batch Processing:** Process multiple samples in parallel using Snakemake

- **🔄 효율적인 배치 처리:** Snakemake를 사용하여 여러 샘플을 병렬로 처리- **🐧 HPC-Ready:** Full support for SLURM, PBS, and other cluster schedulers

- **🔗 DE 파이프라인 통합:** RNA-Seq DE 분석 결과를 자동으로 GSEA 형식으로 변환

- **🎯 Classic GSEA 지원:** Prerank 및 Classic 모드 모두 지원## 📋 Table of Contents

- **🐧 HPC 지원:** SLURM, PBS 등 클러스터 스케줄러 완벽 지원

- [Features](#-features)

## 📋 목차- [Architecture](#-architecture)

- [Installation](#-installation)

- [기능](#-기능)- [Quick Start](#-quick-start)

- [시스템 구조](#-시스템-구조)- [Usage Guide](#-usage-guide)

- [설치](#-설치)- [Snakemake Workflow Automation](#-snakemake-workflow-automation)

- [빠른 시작](#-빠른-시작)- [Configuration](#-configuration)

- [DE 파이프라인 통합](#-de-파이프라인-통합)- [CLI Reference](#-cli-reference)

- [사용 가이드](#-사용-가이드)- [Project Structure](#-project-structure)

  - [GSEA 분석 모드](#-gsea-분석-모드)- [Contributing](#-contributing)

- [Snakemake 워크플로우](#-snakemake-워크플로우)- [Support](#-support)

  - [배치 GSEA 워크플로우](#-배치-gsea-워크플로우)
...
## ✨ 기능- **📥 Data Loading & Standardization:** Load from Excel with flexible column mapping

- **🔍 Gene Filtering:** Filter DEGs by statistical thresholds or custom gene lists

### 분석 기능- **🧬 GO Enrichment Analysis:** Comprehensive GO term enrichment using GOATOOLS

- **📥 데이터 로딩 및 표준화:** Excel 파일에서 유연한 컬럼 매핑으로 데이터 로드- **🎯 GSEA:** Pre-ranked and classic GSEA with MSigDB or custom gene sets

- **🔍 유전자 필터링:** 통계적 임계값 또는 커스텀 유전자 리스트로 DEG 추출- **📈 Visualization Suite:**

- **🧬 GO 농축 분석:** GOATOOLS를 사용한 포괄적인 GO term 농축 분석  - Volcano plots with customizable annotation

- **🎯 GSEA:** Pre-ranked 및 Classic GSEA (MSigDB 또는 커스텀 gene set 지원)  - GO term bar plots and dot plots

- **📈 시각화 도구:**  - GSEA enrichment plots

  - Volcano plot (맞춤형 annotation)  - Multi-condition comparison heatmaps

  - GO term bar plot 및 dot plot- **📄 Automated Reporting:** Generate comprehensive HTML reports with figures and tables

  - GSEA enrichment plot

  - 다중 조건 비교 heatmap### Workflow Features

- **📄 자동 리포트 생성:** 그래프와 표를 포함한 종합 HTML 리포트- **🔧 Modular Design:** Each analysis step is an independent, reusable module

- **📝 YAML Configuration:** All parameters controlled via human-readable config files

### 워크플로우 기능- **📓 Interactive Notebooks:** Step-by-step guided analysis for exploration

- **🔧 모듈식 설계:** 각 분석 단계가 독립적이고 재사용 가능한 모듈- **🐍 Snakemake Workflows:** Automated pipeline execution with dependency tracking

- **📝 YAML 설정:** 모든 파라미터를 읽기 쉬운 설정 파일로 관리- **⚡ Parallel Processing:** Leverage multiple cores for faster batch analysis

- **📓 인터랙티브 노트북:** 단계별 가이드 분석- **🖥️ CLI Tools:** Full command-line interface for all analysis steps

- **🐍 Snakemake 워크플로우:** 의존성 추적을 통한 자동화된 파이프라인 실행- **🔄 Reproducibility:** Configuration-based workflows ensure reproducible results

- **⚡ 병렬 처리:** 다중 코어를 활용한 빠른 배치 분석- **💾 Smart Caching:** Only re-run steps when inputs change (Snakemake)

- **🖥️ CLI 도구:** 모든 분석 단계에 대한 완전한 명령줄 인터페이스

- **🔄 재현성:** 설정 기반 워크플로우로 재현 가능한 결과 보장## 🏗️ Architecture

- **💾 스마트 캐싱:** 입력이 변경될 때만 단계를 재실행 (Snakemake)

The pipeline follows a **modular, Unix-philosophy design** where each analysis step:

## 🏗️ 시스템 구조- Is a self-contained Python script with its own CLI

- Reads configuration from YAML files

파이프라인은 **모듈식, Unix 철학 설계**를 따르며 각 분석 단계는:- Produces well-defined outputs (CSV, plots, reports)

- 자체 CLI를 가진 독립적인 Python 스크립트- Can be run independently or as part of a pipeline

- YAML 파일에서 설정을 읽음

- 명확하게 정의된 출력 생성 (CSV, plot, report)```

- 독립적으로 실행하거나 파이프라인의 일부로 실행 가능Input Data (Excel) → Data Loading → Filtering → Analysis (GO/GSEA) → Visualization → Report

                          ↓             ↓             ↓                    ↓

```                        CSV          CSV         Results CSV           PNG/HTML

입력 데이터 (Excel) → 데이터 로딩 → 필터링 → 분석 (GO/GSEA) → 시각화 → 리포트```

                          ↓            ↓           ↓                 ↓

                        CSV         CSV      Results CSV         PNG/HTMLEach module can be:

```- **Used standalone** via command line

- **Chained together** in notebooks for interactive analysis

각 모듈은:- **Orchestrated** by the batch runner for high-throughput processing

- **독립 실행** 가능 (명령줄 사용)

- **노트북에서 연결** 가능 (인터랙티브 분석)## 🚀 Installation

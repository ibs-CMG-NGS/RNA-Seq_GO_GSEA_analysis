...
1. **Default Templates** (`workflow/config/`)
...
Snakemake does:
1. Loads `workflow/config/go_config.yaml` (defaults from `configfile:` in Snakefile)
...
All other parameters will use defaults from `workflow/config/go_config.yaml`.
...
snakemake --snakefile /full/path/to/workflow/Snakefile_GO ...
...
**Usage**: `snakemake --snakefile workflow/Snakefile_GO --configfile configs/GO_pipeline_Shank2.yaml`  
...
3. **Don't edit `workflow/config/` templates** - they're for defaults only
4. **Use meaningful config names** - e.g., `GO_pipeline_Shank2_3weeks.yaml`
5. **Comment your configs** - future you will thank you
6. **Test with dry-run first**: `snakemake -n --snakefile workflow/Snakefile_GO --configfile configs/YOUR_CONFIG.yaml`
...
| **Template location** | `workflow/config/*.yaml` (read-only) |
| **Path convention** | Relative to project root |
| **Snakefile location** | `workflow/Snakefile_*` (don't move) |
